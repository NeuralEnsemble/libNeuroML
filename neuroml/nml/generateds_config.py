...
# Print out that this is running
# Otherwise generateds silently proceeds, and our nml.py file is all wrong
print("generateds_config.py is being processed")
...
# overriding change to "morphologies" because it only applies outside of a cell
# - not a very elegant solution
NameTable['morphology'] = 'morphology'
...
nametable_fn = "name_table.csv"
print("Saving NameTable to csv file: {}".format(nametable_fn))
with open(nametable_fn, 'w', newline='') as nametable_fh:
    writer = csv.writer(nametable_fh)
    for key, value in list(NameTable.items()):
        writer.writerow([key, value])

changes_fn = "changed_names.csv"
print("Saving name changes table to csv file: {}".format(changes_fn))
with open(changes_fn, 'w', newline='') as changes_fh:
    changes_writer = csv.writer(changes_fh)
